                        ...
                        <div class="p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-between" id="user-{{ $user->id }}">
                            ...
                            <div class="flex gap-2" id="user-actions-{{ $user->id }}">
                                ...
                                @elseif($status['status'] === 'pending')
                                    @if($status['is_sender'] ?? false)
                                        <button onclick="cancelRequest({{ $status['friendship']->id }}, {{ $user->id }})" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                                            Cancel Request
                                        </button>
                                    @else
...
<script>
    ...
    async function sendRequest(userId) {
        try {
            ...
            if (data.success) {
                updateUserActions(userId, 'pending_sent', data.friendship?.id);
            }
        } catch (error) {
    ...
    async function acceptRequest(requestId, userId) {
        try {
            ...
            const data = await response.json();
            if (data.success) {
                updateUserActions(userId, 'friends');
            }
        } catch (error) {
            console.error('Error:', error);
        }
    ...
    function updateUserActions(userId, status, friendshipId = null) {
        ...
        switch(status) {
            case 'friends':
                html = `
                    <a href="/profile/${userId}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        View Profile
                    </a>
                    <button onclick="removeFriend(${userId})" class="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg text-sm font-medium transition-colors">
                        Remove
                    </button>
                `;
                break;
            case 'pending_sent':
                html = `
                    <button onclick="cancelRequest(${friendshipId}, ${userId})" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        Cancel Request
                    </button>
                `;
                break;
            ...
            default:
                html = `
                    <button onclick="sendRequest(${userId})" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
                        Add Friend
                    </button>
                `;
                break;
