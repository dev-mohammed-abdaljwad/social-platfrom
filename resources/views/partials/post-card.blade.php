    ...
    <div class="flex gap-4">
        ...
        <div class="flex-1">
            ...
            <div class="flex items-center gap-6 mt-4 pt-4 border-t border-gray-100">
                ...
                @auth
                <button class="flex items-center gap-2 text-gray-600 hover:text-yellow-500 transition-colors save-btn ml-auto {{ $post->isSavedBy(auth()->user()) ? 'text-yellow-500' : '' }}" data-post-id="{{ $post->id }}">
                    <svg class="w-5 h-5" fill="{{ $post->isSavedBy(auth()->user()) ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
