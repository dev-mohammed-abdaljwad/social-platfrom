            ...
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="relative">
                    ...
                    <div class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 hidden z-10 post-menu" data-post-id="{{ $post->id }}">
                        <button class="w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 rounded-t-lg edit-post-btn" data-post-id="{{ $post->id }}" data-content="{{ $post->content }}" data-location="{{ $post->location }}" data-privacy="{{ $post->privacy?->value }}">
            ...
            <div class="flex items-center justify-between sm:justify-start gap-2 sm:gap-6 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100">
                <div class="flex items-center gap-0.5 sm:gap-1 relative">
                    ...
                    <div class="relative reaction-picker-container" data-post-id="{{ $post->id }}">
                        ...
                        <button class="flex items-center gap-1 sm:gap-2 text-gray-600 hover:text-blue-500 transition-colors reaction-btn-main px-2 py-1.5 rounded-lg hover:bg-gray-100 {{ $userReaction ? 'text-blue-600 font-semibold' : '' }}" data-post-id="{{ $post->id }}" type="button">
                            @if($userReaction)
                        ...
                        <div class="absolute bottom-full left-0 mb-2 bg-white rounded-full shadow-xl border border-gray-100 p-2 hidden reaction-picker z-20 flex items-center gap-1" id="picker-{{ $post->id }}">
                            ...
                            <button class="p-2 hover:scale-125 transition-transform reaction-option" data-post-id="{{ $post->id }}" data-type="{{ $case->value }}" title="{{ ucfirst($case->value) }}" type="button">
            ...
            <div class="comments-section hidden mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100" id="comments-{{ $post->id }}">
                ...
                <form class="comment-form flex gap-2 sm:gap-3 mb-3 sm:mb-4" data-post-id="{{ $post->id }}">
                    ...
                    <div class="flex-1 flex gap-1.5 sm:gap-2">
                        <input type="text" name="content" placeholder="Write a comment..." class="flex-1 px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-100 rounded-full text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
